# -*- coding: utf-8 -*-
"""
Created on Thu Dec  1 15:43:13 2022

@author: fzai683
"""

def main_plot_functionality(functionality, save_dir, p_gantt):
    ...
    full_repair_time = np.nanmax(np.array(schedule['repair_complete_day']['per_story']), axis=1)
    ...
    if len(np.where(fr_time == np.percentile(fr_time,p_gantt))[0])>0:
        ...
        p_idx = np.where(fr_time - np.percentile(fr_time,p_gantt) == min(diff))[0][0]
        from scipy.stats import percentileofscore
        p_gantt = percentileofscore(fr_time, fr_time[p_idx])
    
        
    plot_name = 'prt_'+str(p_gantt)
    other_plot_functions.plt_gantt_chart(p_idx, recovery, full_repair_time, workers, schedule, impede, plot_dir, plot_name)
