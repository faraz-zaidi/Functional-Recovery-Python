...
def fn_calculate_reoccupancy(damage, damage_consequences, utilities, 
    ...
    recovery_day['building_safety'], comp_breakdowns['building_safety'], system_operation_day = other_functionality_functions.fn_building_safety (damage, building_model, 
    ...
    day_building_safe = np.maximum(recovery_day['building_safety']['red_tag'], np.maximum(recovery_day['building_safety']['entry_door_access'], recovery_day['building_safety']['hazardous_material']))
    
    # Check the day each story is accessible
    day_story_accessible = np.maximum(recovery_day['story_access']['stairs'], recovery_day['story_access']['stair_doors'])
    
    # Check the day each tenant unit is safe
    day_tenant_unit_safe = np.maximum(recovery_day['tenant_safety']['interior'], np.maximum(recovery_day['tenant_safety']['exterior'], recovery_day['tenant_safety']['hazardous_material']))
    
    # Combine checks to determine when each tenant unit is re-occupiable
    day_tenant_unit_reoccupiable = np.maximum(np.maximum(day_building_safe.reshape(len(day_building_safe),1), day_story_accessible), day_tenant_unit_safe)
